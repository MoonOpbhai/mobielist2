/* app.js?v=moon-ui — Logic only, no injected CSS */
...
const DEFAULT_SECTIONS = [
  'Movies','Series','Anime','Korean','Bengali',
  'Comedy','Hollywood Comedy','Dark Comedy','Best Webseries','Extra Mentions'
];

/* ── Boot ── */
if (SUPABASE_URL.includes('PASTE_') || SUPABASE_ANON_KEY.includes('PASTE_')) {
  document.getElementById('loading').innerHTML =
    '<span class="state-icon">❌</span><span>Supabase config missing. Render env vars ya public/config.js set karo.</span>';
} else {
...
function detectSection(name) {
  ...
  if (['comedy','21 jump street','horrible bosses','pineapple express',
       'white chicks','tropic thunder'].some(x => n.includes(x))) return 'Comedy';
...
function render() {
  ...
  document.getElementById('seriesCount').textContent = all.filter(m =>
    (m.section||'Movies') === 'Series' || (m.section||'').includes('Webseries')
  ).length;
...
async function delMovie(id) {
  ...
  try {
    const { data, error } = await db.rpc('admin_delete_movie', {
      p_id: id, p_password: adminPassword
    });
    if (error) throw error;
